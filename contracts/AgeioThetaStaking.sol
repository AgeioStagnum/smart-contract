...
contract AgeioStaking is Ownable, ReentrancyGuard {
  ...
  function earnedRewards(address account) public view returns (uint256 tfuelEarned, uint256 agtEarned) {
    tfuelEarned = earned(account);
    agtEarned = 0;
    if (tfuelEarned > 0) {
  ...
  function notifyRewardAmount(uint256 _requestWithdrawals) external payable onlyRewardsDistribution updateReward(address(0)) {
    if (_requestWithdrawals > 0) {
      require(_requestWithdrawals == requestWithdrawals, "notifyRewardAmount: invalid unstaking balance.");
      stakingToken.safeTransferFrom(address(_msgSender()), address(this), requestWithdrawals);
  ...
  event ChangedSettings(uint256 tankFee, uint256 rewardsDuration, uint256 bonusMultiplier);
